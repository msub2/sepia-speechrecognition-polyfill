...
// looking for separate files in a modules folder
// The parts where I basically just shove them in are marked with HACK
import VADWorker from 'web-worker:./webrtc-vad.worker.js';
import STTSocketWorker from 'web-worker:./stt-socket.worker.js';
import SpeexResampleSwitch from 'inline!lib/speex-resample-switch.js';

if (!(typeof SepiaFW == "object")){
	var SepiaFW = {};
	window.SepiaFW = SepiaFW;
}
				...
				options.modules.forEach(async function(module, i){
					...
					//Web Worker
						...
						if (moduleName.includes('webrtc-vad')) {
							thisProcessNode = VADWorker();
						} else if (moduleName.includes('stt-socket')) {
							thisProcessNode = STTSocketWorker();
						}
						thisProcessNode.isReady = false;
						thisProcessNode.moduleName = moduleName;
